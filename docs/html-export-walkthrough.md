...
## Default tags, not just authored ones

`semantics` stays `null` on every node until someone explicitly sets it — a node type's tag was previously resolved ad hoc, each call site guessing its own fallback (`node.semantics?.tag ?? "div"`, `?? "img"`, ...). That's now `DEFAULT_SEMANTIC_TAG` and `resolveSemanticTag()` in `@open-canvas/schema` — one exhaustive, mapped-type table (every `SceneNode["type"]` must have an entry, same pattern as `nodeKinds.ts`), consulted here in `nodeToHtmlElement.ts` and by the properties panel's `SemanticsSection` (now always visible, showing "(default)" next to the resolved tag when nothing's been explicitly authored). The table only gets consulted as a fallback, never written into stored node data — `semantics !== null` still unambiguously means someone explicitly set it, not "happens to match the generic default," and a resolver-at-use-time is needed regardless to handle nodes already saved with `semantics: null`.
...
- A real exported frame overflowed its own declared width in a browser. Two compounding causes, both now fixed with a small reset block in the document `<head>`: any container with both an explicit `width`/`height` and padding (the frame itself, here) rendered *wider than declared* under the browser's default `content-box` box model (`box-sizing: border-box` on every element fixes that), and every element's own UA-default margin adds unwanted space on top — reset to `margin: 0` universally, not just on `body`. Confirmed via `getBoundingClientRect()` that the frame now renders at exactly its declared size with no scroll overflow at a viewport wider than the frame.
- Once text nodes started defaulting to `<p>` (see "Default tags, not just authored ones" above), a text node inside a pill-shaped background overflowed it — `<p>`'s own default `1em` vertical margin, which `<div>` never had. Exactly why the margin reset above is universal (`*`) rather than scoped to `body`.
